    ...
    for i in List.range result.length do
      ...
      | some seg =>
        ...
        | .comp (.iso_hom iso1) (.iso_hom iso2) =>
          let newIso := ExprSegment.iso_hom (mkApp2 (mkConst isoTrans) iso1 iso2)
          newResult := newResult ++ [newIso]
          changed := true
        | .comp (.iso_inv iso1) (.iso_inv iso2) =>
          let newIso := ExprSegment.iso_inv (mkApp2 (mkConst isoTrans) iso2 iso1)
          newResult := newResult ++ [newIso]
          changed := true
    ...
    for i in List.range result.length do
      ...
      | some seg =>
        ...
        | .functor_map F (.iso_hom iso) =>
          let newIso := ExprSegment.iso_hom (mkApp2 (mkConst functorMapIso) F iso)
          newResult := newResult ++ [newIso]
          changed := true
        | .functor_map F (.iso_inv iso) =>
          let newIso := ExprSegment.iso_inv (mkApp2 (mkConst functorMapIso) F iso)
          newResult := newResult ++ [newIso]
          changed := true
        | _ =>
          newResult := newResult ++ [seg]
...
def extractIso (expr : Expr) : MetaM (Option Expr) := do
  if let some iso := asIsoHom? expr then
    return some iso
  if let some iso := asIsoInv? expr then
    return some iso
  return none
