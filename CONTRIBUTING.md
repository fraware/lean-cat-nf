...
- [elan](https://github.com/leanprover/elan) with Lean matching [`lean-toolchain`](lean-toolchain) (currently **v4.31.0-rc1**, same tag as Mathlib in [`Lakefile.lean`](Lakefile.lean)).
- Git
...
Upstream [`doc-gen4`](https://github.com/leanprover/doc-gen4) may not publish a tag that matches every Lean version. For **Lean 4.31** you can either move the project to a Lean pair that has a matching doc tool, or use a separate small Lake project that depends on this package and a compatible doc revision.
...
Linking `lake exe test-runner` or `lake exe bench` can fail on Windows with **undefined symbol: WinMain** (GUI subsystem) or **error 87** (linker command line too long). The Lakefile sets `-Wl,-subsystem,console` on Windows for test and bench executables. Linux CI does not hit the long-command-line issue. Locally you can use a shorter project path, WSL, or Docker. To typecheck tests without building the standalone test executable, run:
